'''
     11/12/2018: Classfication in heuristic way by maximing over mf-e counts.
'''
...
if __name__=="__main__":
    ...
    for qid in qa_list:
        ...
        mfe_dict = gen_utils.read_dict_from_pkl(feat_fn)
        most_common=mfe_dict['most_common']

        cur_prob = [];
        max_prob = -1;
        max_label = None
        for cand_term in candidate_list:
            if cand_term==most_common:
                cur_prob.append(1)
                max_label=cand_term
            else:
                cur_prob.append(0)
